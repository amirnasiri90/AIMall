...
#### تشخیص: سنگینی از منابع سرور است یا از ابر آروان؟

برای فهمیدن اینکه کندی از **سرور** (CPU/رم کم، دیتابیس، Nginx) است یا از **شبکه/ابر آروان** (CDN، مسیر، تایم‌اوت):

**۱. بررسی منابع روی سرور**

روی سرور SSH بزن و یک بار این دستورات را اجرا کن (یا از اسکریپت `scripts/check-server-resources.sh` استفاده کن):
...
- اگر **load average** (مثل `1.5 2.0 1.8`) نزدیک یا بالاتر از تعداد هسته‌های CPU باشد، یا **رم تقریباً پر** باشد و **swap** زیاد استفاده شود، احتمالاً مشکل از **منابع سرور** است.
- اگر load و رم سالم هستند، مرحلهٔ بعد را انجام بده.

**۲. مقایسهٔ زمان پاسخ: مستقیم به سرور vs از طریق دامنه (آروان)**

از **همان سرور** یا از **کامپیوتر خودت** زمان پاسخ را اندازه بگیر:

- **مستقیم به خود سرور (بدون آروان):**  
  اگر روی سرور هستی:
  ```bash
  curl -o /dev/null -s -w "time_total: %{time_total}s\n" http://127.0.0.1:3001/api/v1/health
  curl -o /dev/null -s -w "time_total: %{time_total}s\n" http://127.0.0.1:3000/
  ```
  اگر از بیرون تست می‌کنی، به **IP سرور** درخواست بزن (نه دامنه):
  ```bash
  curl -o /dev/null -s -w "time_total: %{time_total}s\n" http://IP_سرور/api/v1/health
  curl -o /dev/null -s -w "time_total: %{time_total}s\n" http://IP_سرور/
  ```
  (اگر Nginx فقط روی دامنه است، با IP شاید به Nginx/default برخورد کنی؛ در هر صورت زمان را یادداشت کن.)

- **از طریق دامنه (مسیر آروان):**
  ```bash
...
| وضعیت | احتمال |
|--------|--------|
| مستقیم (یا با IP) **سریع** (مثلاً زیر ۱ ثانیه)، دامنه **کند** | مشکل بیشتر از **شبکه یا ابر آروان** (مسیر، CDN، تایم‌اوت، فایروال آروان). |
| هر دو **کند** | مشکل از **سرور** (CPU/رم، Nginx، دیتابیس، کد). اول منابع و لاگ‌ها را چک کن. |
| هر دو **سریع** در تست ولی در مرورگر کند | ممکن است به **فرانت** (JS، درخواست‌های زیاد، کش مرورگر) یا به **اتصال کاربر** مربوط باشد. |

**اگر در تب Network مرورگر اولین درخواست‌ها (مثلاً چند اسکریپت/سی‌اس‌اس) چندصد میلی‌ثانیه تا حدود یک ثانیه طول بکشند و درخواست‌های بعدی همان صفحه خیلی سریع (چند ms) باشند:** تأخیر از **TTFB / برقراری اتصال** است، نه از حجم فایل. در این پروژه در layout فرانت `preconnect` و `dns-prefetch` به همان دامنه اضافه شده تا مرورگر اتصال را زودتر باز کند. روی سرور و CDN (مثلاً آروان) حتماً **HTTP/2** را فعال کن تا یک اتصال برای همهٔ درخواست‌ها multiplex شود و فقط اولین درخواست هزینهٔ handshake را بدهد. در Nginx در بلوک `server` خط `listen 443 ssl;` را به `listen 443 ssl http2;` تغییر بده و بعد `nginx -t` و `reload` کن.

بعد از تشخیص، اگر مشکل از آروان بود: پنل آروان را برای تایم‌اوت، کش و وضعیت CDN چک کن. اگر از سرور بود: طبق بندهای ۱ تا ۴ همین بخش (تایم‌اوت Nginx، رم/CPU، دیتابیس، بهینه‌سازی کد) عمل کن.

### صفحه روی لودینگ گیر می‌کند (اسکلتون بی‌پایان)

اگر بعد از ورود، داشبورد فقط اسکلتون لودینگ نشان می‌دهد و باز نمی‌شود:

۱. **دکمه «ورود مجدد»:** بعد از حدود ۴ ثانیه زیر لودینگ یک دکمه «ورود مجدد» ظاهر می‌شود. با کلیک روی آن به صفحهٔ ورود می‌روی و می‌توانی دوباره لاگین کنی.

۲. **آدرس API در پروداکشن:** در سرور، در فایل `frontend/.env.local` یا محیط build فرانت، **نباید** `NEXT_PUBLIC_API_URL` را روی `http://127.0.0.1:3001` بگذاری؛ چون در مرورگر کاربر این آدرس به ماشین خودش اشاره می‌کند نه به سرور. برای پنل روی یک دامنه (مثلاً panel.aifoapp.ir) که Nginx هم `/api/v1` را به بک‌اند پروکسی می‌کند، بهتر است `NEXT_PUBLIC_API_URL` را **خالی بگذاری** تا فرانت از مسیر نسبی `/api/v1` استفاده کند. بعد از تغییر env حتماً فرانت را دوباره build کن:  
   `cd frontend && rm -rf .next && npm run build` و بعد `pm2 restart aimall-frontend`.

۳. **بک‌اند و Nginx:** اگر بک‌اند جواب ندهد یا Nginx تایم‌اوت کند، درخواست `/api/v1/auth/me` طول می‌کشد و تا ۸ ثانیه لودینگ می‌مانی؛ بعد یا خطا می‌خوری و به ورود می‌روی یا دکمه «ورود مجدد» را بزن. مطمئن شو بک‌اند با PM2 بالا است (`pm2 list`) و تایم‌اوت Nginx برای `location /api/v1` (مثلاً ۱۲۰ ثانیه) تنظیم شده است (بخش ۵۰۴ بالا).
